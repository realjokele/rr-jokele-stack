...
const comboboxStyles = tv({
  slots: {
    ...
    chevronButton:
      '**:data-[slot=icon]:data-pressed:text-fg **:data-[slot=icon]:text-muted-fg **:data-[slot=icon]:hover:text-fg h-7 w-8 rounded outline-offset-0 active:bg-transparent data-hovered:bg-transparent data-pressed:bg-transparent',
    chevronIcon: 'group-open:text-fg size-4 shrink-0 transition duration-200 group-open:rotate-180',
    clearButton:
      'text-muted-fg data-hovered:text-fg absolute inset-y-0 right-0 flex items-center pr-2 data-focused:outline-hidden',
  },
})
  ...
  return (
    <FieldGroup className="relative pl-0">
      ...
      <Button size="square-petite" appearance="plain" className={chevronButton()}>
        {!context?.inputValue && <Icon name="chevron-down" className={chevronIcon()} />}
      </Button>
