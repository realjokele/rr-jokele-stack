...
const fieldStyles = tv({
  slots: {
    description: 'text-pretty text-sm/6 text-muted-fg',
    label: 'w-fit cursor-default text-sm/6 font-medium text-secondary-fg',
...
const fieldGroupStyles = tv({
  base: [
    'shadow-xs group flex h-10 items-center overflow-hidden rounded-lg border border-input transition duration-200 ease-out',
    'group-data-invalid:focus-within:border-danger group-data-invalid:focus-within:ring-danger/20 relative focus-within:ring-4',
  ...
  return (
    <InputPrimitive
      ...
      className={composeTailwindRenderProps(
        className,
        'outline-hidden data-focused:outline-hidden w-full min-w-0 bg-transparent px-2.5 py-2 text-base text-fg placeholder-muted-fg sm:text-sm/6 [&::-ms-reveal]:hidden [&::-webkit-search-cancel-button]:hidden',
      )}
    />
  )
}
